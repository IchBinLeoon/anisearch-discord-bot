...
class Remove(commands.Cog, name='Remove'):
    ...
    async def cmd_remove(self, ctx):
        ...
        try:
            ...
            anisearch.logger.info('Server: %s | Response: No Profile set' % ctx.guild.name)
        if profile_set:
            ...
            anisearch.logger.info('Server: %s | Response: Removed' % ctx.guild.name)


def setup(client):
    client.add_cog(Remove(client))
    anisearch.logger.info('Loaded extension Remove')


def teardown():
    anisearch.logger.info('Unloaded extension Remove')
