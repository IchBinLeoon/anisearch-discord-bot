...
from animesearch.utils.database.profile import update_anilist_profile
from animesearch.utils.database.profile import select_anilist_profile
from animesearch.utils.database.profile import select_myanimelist_profile
from animesearch.utils.database.profile import select_kitsu_profile
from animesearch.utils.database.profile import update_myanimelist_profile
from animesearch.utils.database.profile import update_kitsu_profile
from animesearch.utils.logger import logger
from animesearch.utils.queries.user_query import SEARCH_USER_QUERY
from animesearch.utils.requests import anilist_request
from animesearch.utils.requests import myanimelist_request
from animesearch.utils.requests import kitsu_request
        ...
        try:
            try:
                ...
                embed = discord.Embed(title='Error', description='An error occurred while setting the MyAnimeList '
                                                                 'Profile `{}`. Try again.'.format(username),
                                      color=0xff0000, timestamp=ctx.message.created_at)
                embed.set_footer(text='Requested by {}'.format(ctx.author), icon_url=ctx.author.avatar_url)
                await ctx.channel.send(embed=embed)
            if user is not None:
