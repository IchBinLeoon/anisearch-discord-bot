...
from animesearch.utils.requests import tracemoe_request
from animesearch.utils.requests import saucenao_request
...
class Image(commands.Cog, name='Image'):
    ...
    async def _trace(self, ctx, url):
        embeds = []
        try:
            data = (await tracemoe_request(url))
    ...
    async def _source(self, ctx, url):
        embeds = []
        try:
            content = (await saucenao_request(url))
