...
from animesearch import bot, config
from animesearch.utils.database.prefix import select_prefix
...
class Help(commands.Cog, name='Help'):
    ...
    async def cmd_help(self, ctx, cmd: Optional[str]):
        ...
        if cmd is None:
            embed = discord.Embed(title='AnimeSearch',
                                  description=f'{server_prefix}'
                                              ...
                                              f'\n'
                                              f'**Links:**\n'
                                              f'[Invite AnimeSearch!](https://discord.com/oauth2/authorize?client_id=737236600878137363&permissions=124992&scope=bot) | '
                                              f'[Vote for AnimeSearch!](https://top.gg/bot/737236600878137363/vote)',
                                  color=0x4169E1)
    ...
    async def cmd_commands(self, ctx):
        ...
        embed.set_author(name="AnimeSearch's commands".format(self.bot.user.name), icon_url=self.bot.user.avatar_url)
        await ctx.send(embed=embed)
    ...
    async def cmd_about(self, ctx):
        ...
        embed = discord.Embed(title='About AnimeSearch',
        ...
        embed.add_field(name='❯ Vote', value='[Click me!](https://top.gg/bot/737236600878137363/vote)',
                        inline=True)
        embed.add_field(name='❯ GitHub', value='[Click me!](https://github.com/IchBinLeoon/AnimeSearch)',
                        inline=True)
        embed.set_thumbnail(url=self.bot.user.avatar_url)
        embed.set_footer(text='Requested by {}'.format(ctx.author), icon_url=ctx.author.avatar_url)
        await ctx.channel.send(embed=embed)
    ...
    async def cmd_stats(self, ctx):
        ...
        embed = discord.Embed(description='The current instance of the Bot is owned '
                              ...
                              color=0x4169E1, timestamp=ctx.message.created_at)
        embed.set_author(name="AnimeSearch's statistics".format(self.bot.user.name), icon_url=self.bot.user.avatar_url)
        ...
        try:
            ...
            uptime = '-'
        embed.add_field(name="AnimeSearch's Uptime", value=uptime, inline=True)
        embed.set_footer(text='Requested by {}'.format(ctx.author), icon_url=ctx.author.avatar_url)
        await ctx.channel.send(embed=embed)
