...
class ErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            logger.info('Server: %s | Author: %s | Try: %s' % (ctx.guild.name, ctx.author, ctx.message.content))
            logger.info('Server: %s | Response: Command not found' % ctx.guild.name)

        if isinstance(error, commands.CommandOnCooldown):
            error_embed = discord.Embed(title='The command is on cooldown for `{:.2f}s`'.format(error.retry_after),
                                        color=0xff0000)
            await ctx.channel.send(embed=error_embed)
            logger.info('Server: %s | Response: Command on cooldown' % ctx.guild.name)
        ...
        if isinstance(error, commands.TooManyArguments):
            ...
            logger.info('Server: %s | Response: Too many arguments' % ctx.guild.name)
        ...
        if isinstance(error, commands.MissingPermissions):
            error_embed = discord.Embed(title='Missing permissions',
                                        color=0xff0000)
            await ctx.channel.send(embed=error_embed)
            logger.info('Server: %s | Response: Missing permissions' % ctx.guild.name)
