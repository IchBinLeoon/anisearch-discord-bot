    ...
    async def cmd_setprofile(self, ctx, site: Optional[str], username: Optional[str]):
        ...
        if site:
            if username:
                if site == 'AniList' or site == 'anilist' or site == 'al':
                        ...
                        async with session.post(api, json={'query': query, 'variables': variables}) as r:
                            if r.status == 200:
                                ...
                                try:
                                    ...
                                    bot.logger.info('Server: %s | Response: Set AniList - %s' % (ctx.guild.name,
                                                                                                 username))
                    ...
                    try:
                        ...
                        try:
                            ...
                            bot.logger.info('Server: %s | Response: Set MyAnimeList - %s' %
                ...
                bot.logger.info('Server: %s | Response: Profiles - %s' % (ctx.guild.name, user_name))
            else:
            ...
            embed.add_field(name="MyAnimeList", value=myanimelist_profile,
                            inline=True)
            await ctx.channel.send(embed=embed)
            bot.logger.info('Server: %s | Response: Profiles - %s' % (ctx.guild.name, ctx.author.name))


def setup(client):
    client.add_cog(SetProfile(client))
    bot.logger.info('Loaded extension SetProfile')


def teardown():
    bot.logger.info('Unloaded extension SetProfile')
