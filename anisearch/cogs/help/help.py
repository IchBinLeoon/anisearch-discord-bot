...
class Help(commands.Cog, name='Help'):
    ...
    async def cmd_help(self, ctx, cmd: Optional[str]):
        ...
        if cmd is None:
            ...
            logger.info('Server: %s | Response: Help' % ctx.guild.name)
        else:
            if command := get(self.client.commands, name=cmd):
                ...
                await ctx.send(embed=help_embed)
                logger.info('Server: %s | Response: Help - %s' % (ctx.guild.name, command))
            else:
                error_embed = discord.Embed(title='The command `%s` does not exist' % cmd,
                                            color=0xff0000)
                await ctx.channel.send(embed=error_embed)
                logger.info('Server: %s | Response: Help command not found' % ctx.guild.name)
    ...
    async def cmd_commands(self, ctx):
        ...
        logger.info('Server: %s | Response: Commands' % ctx.guild.name)
    ...
    async def cmd_flags(self, ctx):
        ...
        await ctx.send(embed=cmds_embed)
        logger.info('Server: %s | Response: Command flags' % ctx.guild.name)
