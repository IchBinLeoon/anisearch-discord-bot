...
class Help(commands.Cog, name='Help'):
        ...
        if cmd is None:
            help_embed = discord.Embed(title=self.client.user.name,
                                       description=f'**Current server prefix:** `{prefix}`\n'
                                                   ...
                                                   f'\n'
                                                   f'**Links:**\n'
                                                   f'[Invite {self.client.user.name}!]({bot.__invite__}) | '
                                                   f'[Vote for {self.client.user.name}!]({bot.__vote__})',
                                       color=0x4169E1)
            help_embed.set_thumbnail(url=self.client.user.avatar_url)
            await ctx.send(embed=help_embed)
            bot.logger.info('Server: %s | Response: Help' % ctx.guild.name)
        else:
            if command := get(self.client.commands, name=cmd):
                ...
                await ctx.send(embed=help_embed)
                bot.logger.info('Server: %s | Response: Help - %s' % (ctx.guild.name, command))
            else:
                error_embed = discord.Embed(title='The command `%s` does not exist' % cmd,
                                            color=0xff0000)
                await ctx.channel.send(embed=error_embed)
                bot.logger.info('Server: %s | Response: Help command not found' % ctx.guild.name)
    ...
    async def cmd_commands(self, ctx):
        ...
        bot.logger.info('Server: %s | Response: Commands' % ctx.guild.name)
    ...
    async def cmd_flags(self, ctx):
        ...
        await ctx.send(embed=cmds_embed)
        bot.logger.info('Server: %s | Response: Command flags' % ctx.guild.name)


def setup(client):
    client.add_cog(Help(client))
    bot.logger.info('Loaded extension Help')


def teardown():
    bot.logger.info('Unloaded extension Help')
