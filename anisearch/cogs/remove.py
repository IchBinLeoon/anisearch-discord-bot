...
class Remove(commands.Cog, name='Remove'):
    ...
    async def cmd_remove(self, ctx):
        ...
        try:
            ...
            bot.logger.info('Server: %s | Response: No Profile set' % ctx.guild.name)
        if profile_set:
            ...
            bot.logger.info('Server: %s | Response: Removed' % ctx.guild.name)


def setup(client):
    client.add_cog(Remove(client))
    bot.logger.info('Loaded extension Remove')


def teardown():
    bot.logger.info('Unloaded extension Remove')
