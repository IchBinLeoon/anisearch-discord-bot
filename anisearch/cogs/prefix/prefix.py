...
class Prefix(commands.Cog, name='Prefix'):
    ...
    async def cmd_prefix(self, ctx, prefix):
        ...
        if len(prefix) > 5:
            ...
            logger.info('Server: %s | Response: Prefix wrong length' % ctx.guild.name)
        else:
            ...
            if prefix == 'as!':
                ...
                logger.info('Server: %s | Response: Prefix resetted' % ctx.guild.name)
            else:
                prefix_embed = discord.Embed(title='%s changed the prefix from `%s` to `%s`' %
                                                   (ctx.author, prefix_old, prefix),
                                             color=0x4169E1)
                await ctx.channel.send(embed=prefix_embed)
                logger.info('Server: %s | Response: Prefix changed from %s to %s' % (ctx.guild.name,
                                                                                         prefix_old, prefix))
