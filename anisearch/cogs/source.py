    ...
    async def cmd_source(self, ctx, source=None):
        ...
        if source is None:
            if ctx.message.attachments:
                ...
                bot.logger.info('Server: %s | Response: No image' % ctx.guild.name)
        else:
            url = source
        if url:
            if not url.endswith(('.jpg', '.png', '.bmp', '.gif', '.jpeg')):
                ...
                await ctx.channel.send(embed=error_embed)
                bot.logger.info('Server: %s | Response: No correct url' % ctx.guild.name)
            else:
                ...
                async with aiohttp.ClientSession() as cs:
                    async with cs.get(saucenao) as r:
                        ...
                        if material or artist or characters or pixiv or danbooru or gelbooru or yandere or konachan or \
                            ...
                            bot.logger.info('Server: %s | Response: Source' % ctx.guild.name)
                        else:
                            error_embed = discord.Embed(title='No source found', color=0xff0000)
                            await ctx.channel.send(embed=error_embed)
                            bot.logger.info('Server: %s | Response: No source found' % ctx.guild.name)


def setup(client):
    client.add_cog(Source(client))
    bot.logger.info('Loaded extension Source')


def teardown():
    bot.logger.info('Unloaded extension Source')
