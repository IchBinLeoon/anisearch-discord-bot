...
from anisearch.utils.queries.anime_query import SEARCH_ANIME_QUERY
from anisearch.utils.requests import anilist_request


async def _search_anime_anilist(ctx, title):
    embeds = []
    try:
        ...
        embed = anilist_request_error_embed(ctx, 'anime', title, exception)
        embeds.append(embed)
        return embeds
        ...
        for anime in data:
            ...
            try:
                ...
                if anime['description']:
                    description = description_parser(anime['description'])
                ...
                if len(external_sites) > 1:
                    embed.add_field(name='Streaming and external sites', value=' | '.join(external_sites), inline=False)
                sites = []
                if anime['siteUrl']:
                ...
                logger.exception(exception)
                embed = anilist_load_embed_error_embed(ctx, 'anime', exception, current_page, pages)
                embeds.append(embed)
        return embeds
...
class Anime(commands.Cog, name='Anime'):
    ...
    async def anime(self, ctx, *, title):
        ...
        async with ctx.channel.typing():
            embeds = await _search_anime_anilist(ctx, title)
            if embeds:
                menu = menus.MenuPages(source=AnimeMenu(embeds), clear_reactions_after=True, timeout=30)
                await menu.start(ctx)
            else:
                embed = anilist_not_found_error_embed(ctx, 'anime', title)
                await ctx.channel.send(embed=embed)


class AnimeMenu(menus.ListPageSource):
