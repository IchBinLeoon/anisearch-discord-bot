    ...
    async def cmd_anilist(self, ctx, username: Optional[str]):
        ...
        if username is None:
            ...
            try:
                ...
                bot.logger.info('Server: %s | Response: No Profile linked' % ctx.guild.name)
                ...
                async with session.post(api, json={'query': query, 'variables': variables}) as r:
                    if r.status == 200:
                        ...
                        try:
                            ...
                            bot.logger.info('Server: %s | Response: AniList - %s' % (ctx.guild.name, data['name']))
...
def setup(client):
    ...
    bot.logger.info('Loaded extension AniList')


def teardown():
    bot.logger.info('Unloaded extension AniList')
