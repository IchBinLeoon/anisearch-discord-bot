...
from anisearch.queries import character_query
                            ...
                            if r.status == 200:
                                ...
                                if json['data']['Page']['characters']:
                                    ...
                                    try:
                                        ...
                                        bot.logger.info('Server: %s | Response: Character Search - %s'
                                                        % (ctx.guild.name, name))
                                ...
                                if json['data']['Page']['characters']:
                                    ...
                                    try:
                                        ...
                                        character_embed.set_footer(text='Requested by %s' % ctx.author,
                                                                   icon_url=ctx.author.avatar_url)
                                        await ctx.channel.send(embed=character_embed)
                                        bot.logger.info('Server: %s | Response: Character Image - %s'
                                                        % (ctx.guild.name, name))
                                    except Exception as e:
                                        error_embed = discord.Embed(
                                            title='An error occurred while searching for the character `%s` with the '
                        ...
                        if json['data']['Page']['characters']:
                            ...
                            try:
                                ...
                                character_embed.set_footer(text='Requested by %s' % ctx.author,
                                                           icon_url=ctx.author.avatar_url)
                                await ctx.channel.send(embed=character_embed)
                                bot.logger.info('Server: %s | Response: Character - %s' % (ctx.guild.name,
                                                                                           data['name']['full']))
                            except Exception as e:
                                error_embed = discord.Embed(
...
def setup(client):
    ...
    bot.logger.info('Loaded extension Character')


def teardown():
    bot.logger.info('Unloaded extension Character')
