                            ...
                            if r.status == 200:
                                ...
                                if json['data']['Page']['staff']:
                                    ...
                                    try:
                                        ...
                                        bot.logger.info('Server: %s | Response: Staff Search - %s'
                                                        % (ctx.guild.name, name))
                            ...
                            if r.status == 200:
                                ...
                                if json['data']['Page']['staff']:
                                    ...
                                    try:
                                        ...
                                        bot.logger.info('Server: %s | Response: Staff Image - %s'
                                                        % (ctx.guild.name, name))
                                    except Exception as e:
                                        error_embed = discord.Embed(
                                            title='An error occurred while searching for the staff `%s` with the '
                        ...
                        if json['data']['Page']['staff']:
                            ...
                            try:
                                ...
                                staff_embed.set_footer(text='Requested by %s' % ctx.author,
                                                       icon_url=ctx.author.avatar_url)
                                await ctx.channel.send(embed=staff_embed)
                                bot.logger.info('Server: %s | Response: Staff - %s' % (ctx.guild.name,
                                                                                       data['name']['full']))
                            except Exception as e:
                                error_embed = discord.Embed(
...
def setup(client):
    ...
    bot.logger.info('Loaded extension Staff')


def teardown():
    bot.logger.info('Unloaded extension Staff')
