        ...
        try:
            with conn.cursor() as cur:
                for entry in data.get('data')['Page']['airingSchedules']:
                    try:
                        cur.execute('INSERT INTO schedule (id, time, episode, romaji, english, image, url, nsfw, origin) '
                                    ...
                                    (entry.get('media')['id'],
                                     ...
                                     entry.get('media')['countryOfOrigin']))
                    except Exception as e:
                        logging.exception(e)
                conn.commit()
        ...
        try:
            ...
            with conn.cursor() as cur:
                for episode in new_episodes:
                    cur.execute(
                        'DELETE FROM schedule WHERE id = %s;', (episode,))
